...
# Exercise 3: A Basic Stateful Firewall
...
**This is an advanced exercise, and it is optional.**
...
The directory with this `p4-src` also contains a skeleton P4 program,
...
file `p4src/build/firewall-p4info.txt` after executing `make p4-build`). Any changes in the P4
...
A register in a P4 program is a mutable storage location that can be
used to store state. In this exercise, we will use two register arrays
to implement the bloom filter. The register arrays are defined as follows:

```p4
register<bit<1>>(32) reg_pos_one;
register<bit<1>>(32) reg_pos_two;
```

The following extern declaration describes a generic block of registers, where the type of the elements stored in each register is an arbitrary T.
...
```p4
register<bit<1>>(32) registerBank;
registerBank = Register<bit<1>>(32);
registerBank.write(0, 1); // write 1 to the first register
bit<1> value = registerBank.read(0); // read the value from the first register
```

Bloom filters are probabilistic data structures that can quickly test if an
element is a member of a set. They can return false positives but never false
negatives. In our case, the set is the set of active TCP connections. An empty Bloom filter is an array of m bits, all initially set to 0. The algorithm used k different hash functions which map an element to one of the m possible array positions. To add an element to the set, we hash it k times to get k indices in the array, and set the bits at those indices to 1. To check if an element is in the set, we hash it k times and check if all the bits at those indices are 1. If any of the bits is 0, the element is not in the set. If all the bits are 1, the element may be in the set (a false positive).
